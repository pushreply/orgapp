...
public class ForgotPasswordController extends Activity {
	...
	class SendNewPassword extends AsyncTask<String, String, String>
	{
		...
		/**
		 * Send a request for a new password. A request is successful if 
		 * the email sent is registered in the Database. Otherwise, there will
		 * be no email sent. For security purpose, email check function is not
		 * implemented. Available email validation is only for user input check.
		 * 
		...
		protected String doInBackground(String... args)
		{
			...
			json = jsonParser.makeHttpsRequest(IUniformResourceLocator.URL.URL_PERSON, "GET", params, ForgotPasswordController.this);
